...
deterministically, compiles SQL in your database's own dialect, runs it, and answers. It opens
...
provider, described in [Sign in to ASK](platform/docs/guides/sign-in.md). There is also an
...
- Your data lives in an enterprise system where the names are codes such as
  `VBAK`, `MATNR` and `WERKS`, and what they *mean* lives in a handful of people's heads.
...
compiles to **that engine's dialect**. Each of these has its own SQL generator and its own
execution adapter, not a generic fallback:
...
| **Run the product**: install it, author a semantic layer, publish it, query it | **[`platform/`](platform/README.md)** | The **Onibex Agentic Semantic Knowledge Platform**: source, the Docker Compose stack, and the [complete manual](platform/docs/README.md). |
...
└── platform/                 ← Onibex Agentic Semantic Knowledge Platform
...
To write SQL, the agent is shown your **semantic layer**, the Data
Products you authored, never the raw schema. Query **results** are sent: once the SQL has run, the rows go to the
...
semantic layer earns the most. A `CREATE TABLE` on PostgreSQL is a valid starting point.
...
- **`platform/`**: the Onibex Agentic Semantic Knowledge Platform, under **PolyForm Strict 1.0.0 OR
