...
| **401 / Unauthorized** | Expired or missing login session, or bad provider credentials. | Sign in again. See [Sign in to ASK](../guides/sign-in.md). If it persists, the administrator should verify the provider credentials (and, on cluster deployments, the identity-provider wiring) in **ASK Setup**. |
