...
# back to localhost: the old build-time defaults were http://localhost:8180 and
# http://localhost:5175, so a deployment that forgot a build argument did not
# fail, it shipped a login page pointing at a host that does not exist, with no
# error anywhere.
...
case "$ASK_AUTH_MODE" in
  ...
  xsuaa)
    ...
    ;;
  none)
    # An explicit, deliberate choice for a local or demo stack with no login.
    # It is spelled out rather than being what you get by leaving a variable
    # unset, which is how the old code reached it.
...
# Optional, so they are only defaulted to empty, which the app reads as absent.
...
export ASK_SETUP_SPA_URL="${ASK_SETUP_SPA_URL:-}"
export ASK_CHAT_URL="${ASK_CHAT_URL:-}"
