...
"""
ask_orchestrator/auth/validator.py
─────────────────────────────────────────────────────────────────────────────
Multi-issuer JWT validator for the ASK Orchestrator.

Duplicated from ask-admin-api/auth/validator.py by design — the two services
are physically separate packages that may evolve independently (different
role namespaces, different audience checks, different bypass policies) without
coupling. The shared logic is intentionally inlined rather than extracted into
a shared library to keep each package self-contained.
...
triggers a fresh fetch via httpx. No Redis / external cache required.
...
def _extract_roles_xsuaa(payload: dict[str, Any]) -> list[str]:
    ...
    and including the last dot (e.g. `ask.ask-orchestrator!t1234.query` → `query`).
    We also keep bare `ask.<role>` forms (e.g. `ask.query` → `query`).
    """
    ...
    for scope in raw_scope.split():
        ...
        # `ask-orchestrator!t1234.query`), take the part after the last dot.
...
def _extract_roles_keycloak(payload: dict[str, Any]) -> list[str]:
    ...
    and reading it would make every real user resolve to ``roles == []``.
    """
...
def require_role(role: str):
    ...
    Usage::
        ...
        async def endpoint(
            _claims: TokenClaims = Depends(require_role("query")),
        ) -> ...:
            ...
    """
