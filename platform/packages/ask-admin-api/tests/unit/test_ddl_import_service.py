"""Iter 6 (CH-6) DDL → YAML mapping — parsing + generate_yaml (fake LLM)
plus §7.1 hardening (pre-validator, prose-tolerant extract, multi-table count)."""
...
def test_generate_yaml_parses_multidoc_and_strips_fences():
    ...
    docs, tokens, warnings = svc.generate_yaml(
        "CREATE TABLE X (...)", layer="bronze", source_system="s4h"
    )
...
def test_generate_yaml_warns_on_multitable_undercount():
    # Input declares 2 tables but the model emits only 1 document → warning.
    svc = DdlImportService(prompts_service=None, llm=_FakeLLM("id: bronze_x\nlayer: bronze"))
    ddl = "CREATE TABLE X (a int);\nCREATE TABLE Y (b int);"
    docs, _, warnings = svc.generate_yaml(ddl, layer="bronze", source_system="s4h")
    assert len(docs) == 1
    assert warnings and "CREATE TABLE" in warnings[0]


def test_generate_yaml_flattens_bare_table_silver():
    # A bare CREATE TABLE has no JOIN → a Silver the model wrongly split into two
    # bronze tables (from `_mara`/`_makt` column suffixes) must be flattened to a
    # single-table Silver so it passes the multi-bronze join_graph invariant.
    yaml_out = (
...
def test_generate_yaml_restores_physical_field_name_from_self_source():
...
def test_generate_yaml_normalizes_gold_fields_too():
    ...
    docs, _, warnings = svc.generate_yaml(ddl, layer="gold", source_system="s4h")
    from ask_knowledge_graph.infrastructure.yaml_serializer import load_yaml_text
...
def test_generate_yaml_keeps_field_name_when_source_points_to_real_origin():
    ...
    docs, _, _ = svc.generate_yaml(ddl, layer="silver", source_system="s4h")
    from ask_knowledge_graph.infrastructure.yaml_serializer import load_yaml_text
...
def test_generate_yaml_injects_source_profile_fragment():
    llm = _CapturingLLM()
    svc = DdlImportService(prompts_service=None, llm=llm)
    svc.generate_yaml("CREATE TABLE X (a int);", layer="bronze", source_system="s4h")
...
def test_generate_yaml_omits_context_block_when_empty():
    llm = _CapturingLLM()
    svc = DdlImportService(prompts_service=None, llm=llm)
    svc.generate_yaml("CREATE TABLE X (a int);", layer="bronze", source_system="s4h", context="  ")
...
def test_generate_yaml_injects_fragment_for_human_label():
    ...
    svc.generate_yaml("CREATE TABLE X (a int);", layer="bronze", source_system="SAP S/4HANA 2023")
...
def test_generate_yaml_retries_on_malformed_then_succeeds():
    ...
    svc = DdlImportService(prompts_service=None, llm=llm)
    docs, tokens, warnings = svc.generate_yaml(
        "CREATE TABLE X (a int);", layer="bronze", source_system="s4h"
    )
...
def test_generate_yaml_warns_truncation_on_large_malformed_doc():
    ...
    svc = DdlImportService(prompts_service=None, llm=llm)
    _, _, warnings = svc.generate_yaml(
        "CREATE TABLE X (a int);", layer="bronze", source_system="s4h"
    )
